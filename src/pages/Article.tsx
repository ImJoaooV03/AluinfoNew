    ...
    try {
      ...
      if (data) {
        const mappedArticle: ArticleType = {
          ...
          author: data.author,
          authorRole: data.author_role || 'Colaborador',
          authorAvatar: data.author_avatar || '',
          readTime: data.read_time || '5 min',
          imageUrl: data.image_url || 'https://img-wrapper.vercel.app/image?url=https://img-wrapper.vercel.app/image?url=https://placehold.co/1200x600?text=Sem+Imagem',
      ...
      // Mensagem de erro amigável se não encontrar na região
      setError('Notícia não encontrada ou indisponível nesta região.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchRelatedNews = async (category: string, currentId: string) => {
    try {
      const { data } = await supabase
        ...
        .eq('status', 'published')
        .eq('region', region) // Filtro Estrito
        .neq('id', currentId)
        .limit(6);
      ...
      if (data) {
        const mappedRelated = data.map((item: any) => ({
          ...
          author: item.author,
          imageUrl: item.image_url || 'https://img-wrapper.vercel.app/image?url=https://img-wrapper.vercel.app/image?url=https://placehold.co/600x400',
          isHighlight: false,
          type: 'news'
        }));
    ...
    <div className="min-h-screen bg-[#f8f9fa] pb-12">
      <div className="bg-white border-b border-gray-200">
        <div className="container mx-auto px-4 py-3">
            <div className="flex items-center gap-2 text-xs text-gray-500 overflow-x-auto whitespace-nowrap">
                ...
                <Link to={`/${region}/noticias`} className="hover:text-primary transition-colors uppercase">{t('news')}</Link>
                <ChevronRight size={12} />
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="w-full mb-8">
            <div className="hidden md:block"><AdSpot position="top_large" className="w-full bg-gray-200" fallbackImage="https://img-wrapper.vercel.app/image?url=https://img-wrapper.vercel.app/image?url=https://placehold.co/1200x150?text=Banner" /></div>
            <div className="block md:hidden"><AdSpot position="top_large_mobile" className="w-full bg-gray-200" fallbackImage="https://img-wrapper.vercel.app/image?url=https://img-wrapper.vercel.app/image?url=https://placehold.co/400x150?text=Banner" /></div>
        </div>
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 py-6 border-t border-gray-100 mt-6">
                        <div className="flex items-center gap-3">
                            {article.authorAvatar ? (
                                <img src={article.authorAvatar} alt={article.author} className="w-10 h-10 rounded-full object-cover border border-gray-200" />
                            ) : (
                                <div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 font-bold border border-gray-300"><User size={20} /></div>
                            )}
                            <div><div className="text-sm font-bold text-gray-900">{article.author}</div><div className="text-xs text-gray-500">{article.authorRole}</div></div>
                        </div>
